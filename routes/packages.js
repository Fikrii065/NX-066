...
// Get packages by game code (string) or numeric game_id
  ...
  try {
    ...
    if (isNumeric) {
      [rows] = await db.query(
        `SELECT p.*, g.name AS game_name FROM packages p
         LEFT JOIN games g ON p.game_id=g.id
         WHERE p.game_id=? AND p.is_active=1 ORDER BY p.sort_order, p.base_price`,
        [code]
      );
    } else {
      [rows] = await db.query(
        `SELECT p.*, g.name AS game_name FROM packages p
         LEFT JOIN games g ON p.game_id=g.id
         WHERE (g.code=? OR g.vip_code=?) AND p.is_active=1 ORDER BY p.sort_order, p.base_price`,
        [code, code]
      );
    }
    res.json({ success: true, packages: rows });
