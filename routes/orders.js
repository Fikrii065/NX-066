...
// GET /api/orders/:orderId/check-payment — aktif cek status ke Tokopay & proses jika sudah paid
router.get('/:orderId/check-payment', async (req, res) => {
  ...
  try {
    ...
    // Kalau sudah paid/expired/failed, return langsung
    if (order.payment_status !== 'unpaid') {
      return res.json({ success: true, payment_status: order.payment_status, topup_status: order.topup_status });
    }

    // Cek status ke Tokopay
    const [settingsRows] = await conn.query(
      "SELECT key_name, value FROM settings WHERE key_name IN ('tokopay_merchant_id','tokopay_secret_key')"
    );
    const settings = Object.fromEntries(settingsRows.map(r => [r.key_name, r.value]));
    const MERCHANT  = settings.tokopay_merchant_id || process.env.TOKOPAY_MERCHANT_ID;
    const SECRET    = settings.tokopay_secret_key  || process.env.TOKOPAY_SECRET_KEY;

    const axios = require('axios');
    const { data } = await axios.get(`https://api.tokopay.id/v1/order/status`, {
      params: { merchant: MERCHANT, secret: SECRET, ref_id: order.order_id },
      timeout: 10000,
    });

    console.log('[CheckPayment] Tokopay status response:', JSON.stringify(data));

    const tokopayStatus = data?.data?.status || data?.status;

    if (tokopayStatus === 'Success' && order.payment_status !== 'paid') {
      // Update paid
      await conn.query("UPDATE orders SET payment_status='paid', paid_at=NOW() WHERE order_id=?", [order.order_id]);
      ...
      try {
        ...
        await conn.query('UPDATE orders SET digiflazz_ref=?, topup_status=? WHERE order_id=?',
          [refId, topupStatus, order.order_id]);

        if (result.status === 'Sukses') {
          await conn.query("UPDATE orders SET sn=?, completed_at=NOW() WHERE order_id=?", [result.sn || null, order.order_id]);
        }

        return res.json({ success: true, payment_status: 'paid', topup_status: topupStatus });
      } catch (dErr) {
        ...
        return res.json({ success: true, payment_status: 'paid', topup_status: 'failed' });
      }
    } else if (tokopayStatus === 'Expired') {
      await conn.query("UPDATE orders SET payment_status='expired' WHERE order_id=?", [order.order_id]);
      return res.json({ success: true, payment_status: 'expired', topup_status: order.topup_status });
    }

    return res.json({ success: true, payment_status: order.payment_status, topup_status: order.topup_status });
  } catch (err) {
    console.error('[CheckPayment] Error:', err.message);
    // Jika Tokopay error, kembalikan status DB saja
