...
router.get('/', async (req, res) => {
  try {
    ...
    // Gunakan vip_code dari DB jika ada, fallback ke code game
    const resolvedGameCode = (gameRow.vip_code && gameRow.vip_code.trim() !== '')
      ? gameRow.vip_code.trim()
      : game;

    console.log(`[check-nickname] game=${game} → vip_code=${resolvedGameCode} userId=${userId} zoneId=${zoneId || '-'}`);

    const result = await checkNickname(resolvedGameCode, userId, zoneId || '');
    res.json(result);
  } catch (err) {
