...
router.get('/digiflazz/pricelist', auth, async (req, res) => {
  try {
    ...
    // Set timeout di res agar Railway tidak kill proses
    res.setTimeout(55000);
    req.setTimeout && req.setTimeout(55000);

    const sign = crypto.createHash('md5').update(username + apiKey + 'pricelist').digest('hex');
    const { data } = await axios.post('https://api.digiflazz.com/v1/price-list',
      { cmd: 'prepaid', username, sign },
      { timeout: 50000, maxContentLength: 50 * 1024 * 1024 }
    );
    const products = Array.isArray(data.data) ? data.data : [];
    res.json({ success: true, products, total: products.length });
  } catch (e) {
    console.error('[pricelist]', e.message);
