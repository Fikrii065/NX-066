...
router.get('/digiflazz/pricelist', auth, async (req, res) => {
  try {
    ...
    // Perpanjang timeout request & response agar tidak di-kill Railway (60s default)
    req.socket && (req.socket.setTimeout(110000));
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        const resp = await axios.post('https://api.digiflazz.com/v1/price-list',
          ...
          { timeout: 90000, maxContentLength: 100 * 1024 * 1024, maxBodyLength: 100 * 1024 * 1024 }
        );
        data = resp.data;
        break;
      } catch (axErr) {
        if (attempt === 2) throw axErr;
        console.warn('[pricelist] attempt', attempt, 'gagal, retry...', axErr.message);
        await new Promise(r => setTimeout(r, 1500));
      }
    }

    const products = Array.isArray(data.data) ? data.data : [];
    if (!res.headersSent) res.json({ success: true, products, total: products.length });
  } catch (e) {
    console.error('[pricelist]', e.code || '', e.message);
    if (!res.headersSent) res.status(500).json({ success: false, message: 'Gagal ambil pricelist: ' + e.message });
  }
});
