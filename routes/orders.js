...
router.post('/create', async (req, res) => {
  ...
  try {
    const { game_id, sku_code, customer_no, payment_method, customer_name, customer_email, customer_wa, discount_code } = req.body;
    ...
    if (discount_code) {
      try {
        ...
        if (dv) {
          const now = new Date();
          const stillValid = (!dv.valid_from || new Date(dv.valid_from) <= now)
                          && (!dv.valid_until || new Date(dv.valid_until) >= now)
                          && (dv.quota === null || dv.used_count < dv.quota)
                          && (dv.min_purchase <= 0 || sellPrice >= parseFloat(dv.min_purchase));
          if (stillValid) {
            if (dv.discount_type === 'percent') {
              discountAmount = (sellPrice * parseFloat(dv.discount_value)) / 100;
              if (dv.max_discount && discountAmount > parseFloat(dv.max_discount)) discountAmount = parseFloat(dv.max_discount);
            } else {
              discountAmount = parseFloat(dv.discount_value);
            }
            discountAmount = Math.round(Math.min(discountAmount, sellPrice));
            total = Math.max(0, total - discountAmount);
            appliedVoucherId = dv.id;
          }
        }
      } catch(dvErr) { console.error('[discount voucher]', dvErr.message); }
    }
    ...
    if (payment_method === 'balance') {
        ...
        await conn.query(
          `INSERT INTO orders
            (order_id, game_id, package_id, customer_no, customer_name, customer_email, customer_wa,
             base_price, sell_price, service_fee, discount_amount, total_amount, discount_code,
          ...
          [orderId, game_id, pkg.id, customer_no, customer_name || null, userRow.email,
           customer_wa || null, pkg.base_price, sellPrice, fee,
           discountAmount || 0, total, discount_code ? discount_code.trim().toUpperCase() : null,
           'balance']
        );
      } catch(insertErr) {
        if (insertErr.code === 'ER_WRONG_VALUE_COUNT_ON_ROW') {
          // Kolom discount belum ada — fallback tanpa discount
          await conn.query(
            `INSERT INTO orders
              (order_id, game_id, package_id, customer_no, customer_name, customer_email, customer_wa,
               base_price, sell_price, service_fee, total_amount,
               payment_method, payment_status, topup_status, paid_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,'paid','pending',NOW())`,
            [orderId, game_id, pkg.id, customer_no, customer_name || null, userRow.email,
      ...
      if (appliedVoucherId) {
        await conn.query('UPDATE discount_vouchers SET used_count = used_count + 1 WHERE id = ?', [appliedVoucherId]);
      }
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [[{ total }]] = await db.query(
      `SELECT COUNT(*) AS total FROM orders o LEFT JOIN games g ON o.game_id = g.id ${where}`,
      params
    );
...
router.post('/:orderId/retry', auth, async (req, res) => {
  try {
    const [[order]] = await db.query(
      `SELECT o.*, p.digiflazz_sku FROM orders o LEFT JOIN packages p ON o.package_id = p.id WHERE o.order_id = ?`,
      [req.params.orderId]
    );
