...
async function autoMigrate() {
  ...
  try {
    ...
    try {
      await db.query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount_code VARCHAR(50) NULL DEFAULT NULL AFTER service_fee');
      await db.query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount_amount INT UNSIGNED NOT NULL DEFAULT 0 AFTER discount_code');
    } catch(e) {
      // MySQL < 8 tidak support IF NOT EXISTS di ALTER TABLE — fallback
      try { await db.query('ALTER TABLE orders ADD COLUMN discount_code VARCHAR(50) NULL DEFAULT NULL'); } catch(_) {}
      try { await db.query('ALTER TABLE orders ADD COLUMN discount_amount INT UNSIGNED NOT NULL DEFAULT 0'); } catch(_) {}
    }
