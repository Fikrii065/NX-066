...
function verifyCallback(payload, secretKey) {
  ...
  if (!incoming || !refId || !merchantId) {
    console.warn('[verifyCallback] Field kurang — incoming:', incoming, '| refId:', refId, '| merchantId:', merchantId);
    return false;
  }

  // Formula resmi Tokopay: md5(merchant_id:secret:ref_id)
  console.log('[verifyCallback] SK prefix:', SK ? SK.substring(0,6)+'...' : 'KOSONG!');
  const expected = crypto.createHash('md5').update(`${merchantId}:${SK}:${refId}`).digest('hex');
