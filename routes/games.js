...
router.get('/', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, code, name, icon, category, params, zone_label, ' +
...
router.get('/all', auth, async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, code, name, icon, category, params, zone_label, vip_code, sort_order, is_active, created_at, ' +
      'CASE WHEN icon_url IS NOT NULL AND icon_url != "" THEN 1 ELSE 0 END AS has_icon, ' +
      'CASE WHEN banner_url IS NOT NULL AND banner_url != "" THEN 1 ELSE 0 END AS has_banner ' +
...
router.post('/', auth, async (req, res) => {
  try {
    const { code, name, icon, icon_url, banner_url, params, zone_label, vip_code, sort_order, category } = req.body;
    if (!code || !name) return res.status(400).json({ success: false, message: 'code dan name wajib diisi' });
    const [result] = await db.query(
      'INSERT INTO games (code, name, icon, icon_url, banner_url, params, zone_label, vip_code, sort_order, category) VALUES (?,?,?,?,?,?,?,?,?,?)',
      [code, name, icon || '🎮', icon_url || null, banner_url || null, params || 'userId', zone_label || null, vip_code || code, sort_order || 0, category || 'game']
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { name, icon, icon_url, banner_url, params, zone_label, vip_code, sort_order, is_active, category } = req.body;
    await db.query(
      'UPDATE games SET name=?, icon=?, icon_url=?, banner_url=?, params=?, zone_label=?, vip_code=?, sort_order=?, is_active=?, category=? WHERE id=?',
      [name, icon, icon_url || null, banner_url || null, params, zone_label, vip_code, sort_order, is_active ? 1 : 0, category || 'game', req.params.id]
    );
