...
// ── Auto-generate JWT_SECRET jika belum diset ────────────────────────────────
const crypto_startup = require('crypto');
const PLACEHOLDER_SECRETS = [
  'GANTI_DENGAN_OUTPUT_PERINTAH_DI_ATAS', 'change_me', 'your_jwt_secret', 'secret', ''
];
if (!process.env.JWT_SECRET || PLACEHOLDER_SECRETS.includes(process.env.JWT_SECRET) || process.env.JWT_SECRET.length < 32) {
  const generated = crypto_startup.randomBytes(32).toString('hex');
  process.env.JWT_SECRET = generated;
  console.log('⚡ JWT_SECRET di-generate otomatis (session ini saja).');
  console.log('   Untuk persistent, set di Railway Variables:');
...
async function autoMigrate() {
  ...
  try {
    conn = await mysql.createConnection({
      ...
      password:           process.env.DB_PASS     || process.env.MYSQLPASSWORD || process.env.MYSQL_PASSWORD || process.env.MYSQL_ROOT_PASSWORD || '',
    ...
    // ── Migrate: tambah kolom discount ke orders jika belum ada ─────────────────
    // Pakai INFORMATION_SCHEMA agar kompatibel semua versi MySQL
    try {
      const dbName = process.env.DB_NAME || process.env.MYSQLDATABASE || 'railway';
      const [[dcRow]] = await db.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'orders' AND COLUMN_NAME = 'discount_code' LIMIT 1`,
        [dbName]
      );
      if (!dcRow) {
        await db.query('ALTER TABLE orders ADD COLUMN discount_code VARCHAR(50) NULL DEFAULT NULL AFTER service_fee');
        console.log('  Migrate: kolom discount_code ditambahkan');
      }
      const [[daRow]] = await db.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'orders' AND COLUMN_NAME = 'discount_amount' LIMIT 1`,
        [dbName]
      );
      if (!daRow) {
        await db.query('ALTER TABLE orders ADD COLUMN discount_amount INT UNSIGNED NOT NULL DEFAULT 0 AFTER discount_code');
        console.log('  Migrate: kolom discount_amount ditambahkan');
      }
    } catch(migrateErr) {
      console.error('  Migrate discount columns error:', migrateErr.message);
    }
...
// GET /api/version — cek versi file yang sedang jalan
app.get('/api/version', (req, res) => {
  const fs = require('fs');
  const adminPath = require('path').join(__dirname, 'public', 'admin.html');
  try {
    const first300 = fs.readFileSync(adminPath, 'utf8').substring(0, 300);
    const versionMatch = first300.match(/ADMIN_VERSION:\s*([\w\d\-\.]+)/);
    res.json({
      success: true,
      admin_version: versionMatch ? versionMatch[1] : 'UNKNOWN (file lama, tidak ada version marker)',
      server_has_ping: true,
      admin_first_line: first300.split('\n')[0]
    });
  } catch(e) {
...
Object.entries(pages).forEach(([route, file]) => {
  app.get(route, (_req, res) => {
    // No-cache for HTML pages so browser always fetches latest version
