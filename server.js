...
// ── Body parsers ─────────────────────────────────────────────────────────────
// Untuk webhook Tokopay & Digiflazz: terima raw body dulu, lalu parse manual
// supaya tidak crash kalau payload dikirim sebagai form-urlencoded atau plain string
app.use('/api/webhook', (req, res, next) => {
  // PENTING: jangan gunakan req.setEncoding() — raw-body (dipakai express.json)
  // akan throw "stream encoding should not be set" jika encoding sudah diset.
  // Kumpulkan Buffer chunks, lalu decode manual setelah selesai.
  const chunks = [];
  ...
  req.on('end', () => {
    ...
    try {
      ...
      // Jika bukan JSON, coba parse sebagai URL-encoded form
      try {
