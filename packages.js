...
// Auto-ensure kolom opsional di tabel packages
async function ensurePackageColumns() {
  const cols = [
    'ALTER TABLE packages ADD COLUMN icon_url MEDIUMTEXT NULL',
    'ALTER TABLE packages ADD COLUMN digiflazz_sku VARCHAR(80) NULL',
  ];
  for (const sql of cols) {
    try { await db.query(sql); } catch(e) { /* already exists */ }
  }
}
ensurePackageColumns().catch(() => {});
