...
async function checkNickname(gameCode, userId, zoneId) {
  ...
  if (apiId && apiKey) {
    try {
      // VIP Reseller menggunakan POST form data (x-www-form-urlencoded)
      const params = new URLSearchParams();
      params.append('api_id', apiId);
      params.append('api_key', apiKey);
      params.append('game', gameCode);
      params.append('user_id', userId);
      params.append('zone_id', zoneId || '');
      const { data } = await axios.post('https://vip-reseller.co.id/api/game-feature', params, {
    ...
    } catch (e) { console.error('VIP Reseller error:', e.message); }
  }
