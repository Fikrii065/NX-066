...
router.get('/dashboard', auth, async (req, res) => {
  try {
    // Jalankan query DB saja — TANPA Digiflazz agar tidak blokir
    const [
      [todayRows],
      [yesterdayRows],
      [recentOrders],
      ...
      db.query(`
        SELECT o.order_id,o.created_at,o.total_amount,o.payment_status,o.topup_status,
               COALESCE(g.name,'(Game dihapus)') AS game_name,
               COALESCE(g.icon,'🎮') AS game_icon,
               COALESCE(p.name,'(Paket dihapus)') AS package_name
        FROM orders o
    ...
    // Kirim data DB segera — Digiflazz balance diambil terpisah via /api/admin/digiflazz/balance
    res.json({
      success: true,
      today: todayRows[0],
      yesterday: yesterdayRows[0],
      recent_orders: recentOrders,
      digiflazz_balance: null,
    });
  } catch(err) {
