...
router.post('/', auth, async (req, res) => {
  try {
    const { code, name, icon, icon_url, banner_url, params, zone_label, zone_options, vip_code, sort_order, category } = req.body;
    if (!code || !name) return res.status(400).json({ success: false, message: 'code dan name wajib diisi' });
    const [result] = await db.query(
      'INSERT INTO games (code, name, icon, icon_url, banner_url, params, zone_label, zone_options, vip_code, sort_order, category) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      [code, name, icon || '🎮', icon_url || null, banner_url || null, params || 'userId', zone_label || null, zone_options || null, vip_code || code, sort_order || 0, category || 'game']
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { name, icon, icon_url, banner_url, params, zone_label, zone_options, vip_code, sort_order, is_active, category } = req.body;
    await db.query(
      'UPDATE games SET name=?, icon=?, icon_url=?, banner_url=?, params=?, zone_label=?, zone_options=?, vip_code=?, sort_order=?, is_active=?, category=? WHERE id=?',
      [name, icon, icon_url || null, banner_url || null, params, zone_label, zone_options || null, vip_code, sort_order, is_active ? 1 : 0, category || 'game', req.params.id]
    );
