...
async function checkNickname(gameCode, userId, zoneId) {
  ...
  if (apiId && apiKey) {
    try {
      ...
      // data.data bisa berupa string langsung (nama) atau object
      if (data.result === true) {
        const nickname =
        ...
        console.warn('[VIP Reseller] result=true tapi tidak ada nickname, full response:', JSON.stringify(data));
        return { success: true, nickname: `User ${userId}` };
      }
...
async function createTransaction(refId, digiflazzSku, customerNo) {
  ...
  // Digiflazz ML: customer_no harus format "userID.zoneID" bukan "userID|zoneID"
  // Konversi separator | ke . jika ada
  const formattedCustomerNo = customerNo.includes('|')
    ? customerNo.replace('|', '.')
    : customerNo;

  console.log('[Digiflazz] createTransaction:', {
    ref_id: refId,
    sku: digiflazzSku,
    customer_no: formattedCustomerNo,
    mode,
    testing: mode !== 'production',
  });

  const { data } = await axios.post(`${BASE_URL}/transaction`, {
    username, buyer_sku_code: digiflazzSku, customer_no: formattedCustomerNo,
