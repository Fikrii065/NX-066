...
// ── Body parsers ─────────────────────────────────────────────────────────────
// PENTING: express.json() dan express.urlencoded() harus didaftarkan SEKALI
// untuk semua route — termasuk /api/webhook. Jangan buat middleware stream
// manual di sini karena akan conflict (stream sudah terbaca sehingga
// "stream is not readable" muncul).
// express.json() sudah cukup untuk menangani payload JSON dari Tokopay & Digiflazz.
app.use(express.json({ limit: '10mb' }));
app.use(express.urlencoded({ extended: true, limit: '10mb' }));
// FIX: Static files — HTML files tidak di-cache agar update langsung terlihat
...
// FIX: Explicit route untuk admin.html dengan no-cache header
...
app.use('/api/admin',          require('./routes/admin_extra'));  // flash sale, settings, admins, fonnte, dll
app.use('/api/admin/categories', require('./routes/categories')); // category management
...
app.get('/metode-pembayaran', (req, res) => res.sendFile(path.join(__dirname, 'public', 'metode-pembayaran.html')));
