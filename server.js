...
// HTML files: no cache agar perubahan langsung terlihat
...
async function autoMigrate() {
  ...
  try {
    ...
    // Migrate: paksa ALTER semua kolom yang butuh ukuran besar
    const alters = [
      { sql: 'ALTER TABLE games MODIFY COLUMN icon_url MEDIUMTEXT NULL',   label: 'games.icon_url' },
      { sql: 'ALTER TABLE games MODIFY COLUMN banner_url MEDIUMTEXT NULL', label: 'games.banner_url' },
      { sql: 'ALTER TABLE settings MODIFY COLUMN value MEDIUMTEXT NULL',   label: 'settings.value' },
    ];
    for (const { sql, label } of alters) {
      try {
        await conn.query(sql);
        console.log(`[Migrate] ${label} → MEDIUMTEXT ✅`);
      } catch(e) {
        // Abaikan jika sudah MEDIUMTEXT (error 1060/1061), log sisanya
        if (!['ER_DUP_FIELDNAME','ER_CANT_DROP_FIELD_OR_KEY'].includes(e.code)) {
          console.warn(`[Migrate] ALTER ${label}:`, e.message);
        }
      }
    }
...
app.use('/api/webhook',          require('./routes/webhooks')); // alias
app.use('/api/check-nickname',   require('./routes/check_nickname'));
// server-ip: ambil IP publik Railway untuk whitelist VIP Reseller
const _cnRouter = require('./routes/check_nickname');
app.get('/api/server-ip', (req, res, next) => { req.url = '/server-ip'; _cnRouter(req, res, next); });
