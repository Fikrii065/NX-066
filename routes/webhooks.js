  ...
  try {
    ...
    if (status === 'Success' && order.payment_status !== 'paid') {
      ...
      try {
        ...
        // Set topup_status='failed' dan notes agar invoice langsung tampil status gagal ke buyer
        await conn.query(
          "UPDATE orders SET topup_status='failed', notes=? WHERE order_id=?",
          [`Top up gagal diproses: ${dErrMsg}. Silakan hubungi admin untuk diproses manual.`, orderId]
        );
        console.log('[Webhook Tokopay] Order', orderId, 'marked as failed topup, buyer will see failure on invoice.');
      }
...
router.post('/digiflazz', async (req, res) => {
  ...
  try {
    ...
    // Jika gagal, simpan pesan error ke notes agar buyer langsung tahu di halaman invoice
