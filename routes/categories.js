  ...
  try {
    ...
    await db.query(
      `UPDATE categories SET name=?, slug=?, image_url=?, guide_image_url=?,
       description=?, information=?, has_zone_id=?, sort_order=?, is_active=? WHERE id=?`,
      [name.trim(), finalSlug, finalImage, finalGuideImage, description||null, information||null,
       has_zone_id ? 1 : 0, sort_order !== undefined ? parseInt(sort_order) : 0, is_active ? 1 : 0, id]
    );

    const [[updated]] = await db.query(
      'SELECT id, name, slug, has_zone_id, sort_order, is_active, updated_at FROM categories WHERE id = ?', [id]
    );
