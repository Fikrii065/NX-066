...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [code, name, icon || '🎟️', icon_url || null, link_url || null, category || 'platform', sort_order || 0]
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [name, icon || '🎟️', icon_url || null, link_url || null, category || 'platform', sort_order || 0, is_active ? 1 : 0, req.params.id]
    );
