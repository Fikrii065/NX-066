...
router.get('/all', auth, async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, code, name, icon, category, params, zone_label, zone_options, vip_code, sort_order, is_active, created_at, ' +
      'CASE WHEN icon_url IS NOT NULL AND icon_url != "" THEN 1 ELSE 0 END AS has_icon, ' +
      'CASE WHEN banner_url IS NOT NULL AND banner_url != "" THEN 1 ELSE 0 END AS has_banner ' +
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { name, icon, icon_url, banner_url, link_url, params, zone_label, zone_options, vip_code, sort_order, is_active, category } = req.body;
    await db.query(
      'UPDATE games SET name=?, icon=?, icon_url=?, banner_url=?, link_url=?, params=?, zone_label=?, zone_options=?, vip_code=?, sort_order=?, is_active=?, category=? WHERE id=?',
      [name, icon, icon_url || null, banner_url || null, link_url || null, params, zone_label, zone_options || null, vip_code, sort_order, is_active ? 1 : 0, category || 'game', req.params.id]
    );
