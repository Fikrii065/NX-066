...
router.post('/tokopay', async (req, res) => {
  ...
  try {
    ...
    //   payload.ref_id    → ref_id MERCHANT yang kita kirim saat buat order (format GF...)  ← ini yang kita butuhkan
    //   payload.reff_id   → ID internal Tokopay (format AUTO...)
    //   payload.reference → ID order Tokopay (format TP...) — BUKAN merchant order ID
    // Gunakan payload.ref_id sebagai primary key pencarian order.
    const refId = payload.ref_id || payload.order_id || payload.reference || payload.reff_id;
    if (!refId) {
...
router.post('/tokopay-balance', async (req, res) => {
  ...
  try {
    ...
    // PENTING: Sama seperti /tokopay — gunakan payload.ref_id (merchant ref_id = DEP-...)
    // bukan payload.reference (ID order Tokopay, format TP...) atau payload.reff_id (ID internal Tokopay).
    const refId = payload.ref_id || payload.order_id || payload.reference || payload.reff_id;
    if (!refId) {
      return res.json({ rc: '00', message: 'ok' });
    }
