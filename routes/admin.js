...
router.get('/dashboard', auth, async (req, res) => {
  try {
    ...
    let balance = null;
    try { const bal = await digiflazz.getBalance(); balance = bal?.deposit ?? null; } catch(_) {}
    res.json({ success:true, today, yesterday, recent_orders:recentOrders, digiflazz_balance:balance });
  } catch(err) {
    console.error(err);
...
router.get('/digiflazz/pricelist', auth, async (req, res) => {
  try {
    ...
    const { data } = await axios.post('https://api.digiflazz.com/v1/price-list',{ cmd:'prepaid',username,sign },{ timeout:20000 });
    let products = Array.isArray(data.data) ? data.data : [];
    if (!products.length && data.data && !Array.isArray(data.data)) {
      return res.status(500).json({ success:false, message: (typeof data.data === 'object' ? JSON.stringify(data.data) : String(data.data)) });
    }
...
router.post('/sync-products', auth, async (req, res) => {
  try {
    ...
    const { data: plData } = await axios.post('https://api.digiflazz.com/v1/price-list',
      { cmd: 'prepaid', username, sign }, { timeout: 20000 }
    );
