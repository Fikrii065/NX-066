    ...
    if (status === 'Success' && order.payment_status !== 'paid') {
      ...
      try {
        ...
        if (result.status === 'Sukses') {
          ...
          const orderDone = { ...order, sn: result.sn || null, completed_at: new Date() };
          fonnte.notifyTopupSuccess(orderDone).catch(e => console.error('[Fonnte] notifyTopupSuccess:', e.message));

        } else if (topupStatusWh === 'failed') {
          // ── Notifikasi: Top up gagal (langsung dari Digiflazz) ─────────────
          const orderFailed = { ...order, notes: topupErrorWh };
          fonnte.notifyTopupFailed(orderFailed).catch(e => console.error('[Fonnte] notifyTopupFailed:', e.message));
        }
        ...
        fonnte.notifyTopupFailed({ ...order, notes: failMsg }).catch(e => console.error('[Fonnte] notifyTopupFailed:', e.message));
      ...
      fonnte.notifyPaymentExpired(order).catch(e => console.error('[Fonnte] notifyPaymentExpired:', e.message));
...
router.post('/digiflazz', async (req, res) => {
  ...
  try {
    ...
    if (topupStatus === 'success') {
      ...
      // ── Notifikasi: Top up sukses (via Digiflazz webhook) ─────────────────
      const orderDone = { ...order, sn: data.sn || null, completed_at: new Date() };
      fonnte.notifyTopupSuccess(orderDone).catch(e => console.error('[Fonnte] notifyTopupSuccess:', e.message));

    } else if (topupStatus === 'failed') {
      // ── Notifikasi: Top up gagal (via Digiflazz webhook) ──────────────────
      fonnte.notifyTopupFailed({ ...order, notes: failNotes }).catch(e => console.error('[Fonnte] notifyTopupFailed:', e.message));
    }
